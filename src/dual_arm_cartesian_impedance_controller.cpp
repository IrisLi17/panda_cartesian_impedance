...
#include <panda_cartesian_impedance/dual_arm_cartesian_impedance_controller.h>
...
#include <panda_cartesian_impedance/pseudo_inversion.h>
...
bool DualArmCartesianImpedanceExampleController::init(hardware_interface::RobotHW* robot_hw,
  ...
  ROS_INFO_STREAM(
    "left_arm_id_ = " << left_arm_id_ << " right_arm_id_ = " << right_arm_id_
  );

  boost::function<void(const DualPoseStamped::ConstPtr&)> callback =
      boost::bind(&DualArmCartesianImpedanceExampleController::targetPoseCallback, this, _1);

  ros::SubscribeOptions subscribe_options;
  subscribe_options.init("dual_arm_target_pose", 1, callback);
  subscribe_options.transport_hints = ros::TransportHints().reliable().tcpNoDelay();
  sub_target_dual_poses_ = node_handle.subscribe(subscribe_options);
...
void DualArmCartesianImpedanceExampleController::updateArm(FrankaDataContainer& arm_data) {
  ...
  panda_cartesian_impedance::pseudoInverse(jacobian.transpose(), jacobian_transpose_pinv);
...
void DualArmCartesianImpedanceExampleController::targetPoseCallback(
    const DualPoseStamped::ConstPtr& msg) {
  try {
    if (msg->left_pose.header.frame_id != left_arm_id_ + "_link0") {  // NOLINT
      ROS_ERROR_STREAM(
          "First branch DualArmCartesianImpedanceExampleController: Got pose target with invalid"
          " frame_id "
          << msg->left_pose.header.frame_id << ". Expected " << left_arm_id_ + "_link0");
      return;
    }
    if (msg->right_pose.header.frame_id != right_arm_id_ + "_link0") {
      ROS_ERROR_STREAM(
        "Second branch DualArmCartesianImpedanceExampleController: Got pose target with invalid"
        " frame_id "
        << msg->right_pose.header.frame_id << ". Expected " << right_arm_id_ + "_link0");
      return;
    }
    ...
    tf::poseMsgToEigen(msg->left_pose.pose, Ol_T_EEl_d);
    // Eigen::Affine3d Ol_T_EEl_d =      // NOLINT (readability-identifier-naming)
    ...
    auto& right_arm_data = arms_data_.at(right_arm_id_);
    Eigen::Affine3d Or_T_EEr_d{};
    tf::poseMsgToEigen(msg->right_pose.pose, Or_T_EEr_d);
...
PLUGINLIB_EXPORT_CLASS(panda_cartesian_impedance::DualArmCartesianImpedanceExampleController,
